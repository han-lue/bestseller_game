...
export default function BookRight({title, author, year_published, cover, showAnswer, weeks_number_one, answerIs, handleAnswer}: BookRightPropTypes) {
  ...
  return (
    <div className='w-1/2 h-full flex flex-col items-center justify-end gap-3 relative'>
      { showAnswer && 
        <motion.p initial={{opacity: 0.5}} animate={{opacity: 1}} transition={{duration: 2, ease: "easeOut"}}
          className={`absolute bg-black p-2 c-font-text font-bold rounded-lg left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2`}
